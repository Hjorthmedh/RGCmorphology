...
% These are features we use for classifying, however these were
% optimized for with the knowledge about the genetic labels
% Gives a peak at k=2-3, then drops
%

% Used for the Figure in FigureBlindOraceFeatures
if(1)
...
for k = 1:kMax
  ...
  if(k == 1)
    fName = 'FIGS/Silhouette-measure-genetic-marker-silhouette.pdf';
  else
    fName = sprintf('FIGS/Silhouette-measure-k-%d-silhouette.pdf',k);
  end
  ...
  if(k == 1)
    fName = 'FIGS/Silhouette-measure-genetic-marker-feature-space.pdf';
  else
    fName = sprintf('FIGS/Silhouette-measure-k-%d-feature-space.pdf',k);
  end
  saveas(gcf,fName,'pdf')
...
hr = plot(5,silMean(1),'r.','markersize',20);
...
%saveas(gcf,'FIGS/Silhouette-measure-summary.pdf','pdf')
printA4('FIGS/Silhouette-measure-summary.eps')
...
for nClust = [2 3 5];
  ...
  fName = sprintf('RESULTS/BlindClustering-latex-n-%d.tex', nClust);
  fid = fopen(fName,'w');
  fprintf(fid,str2);
  fclose(fid);
end
