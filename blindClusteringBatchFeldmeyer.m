...
r = RGCclass(0); r.lazyLoad('Feldmeyer');
...
yVar = 'numBranchPoints';
xVar = 'dendriticDensity';
...
for k = 1:kMax
  ...
  if(k == 1)
    fName = 'FIGS/Feldmeyer-Silhouette-measure-genetic-marker-silhouette.pdf';
  else
    fName = sprintf('FIGS/Feldmeyer-Silhouette-measure-k-%d-silhouette.pdf',k);
  end
  ...
  if(k == 1)
    fName = 'FIGS/Feldmeyer-Silhouette-measure-genetic-marker-feature-space.pdf';
  else
    fName = sprintf('FIGS/Feldmeyer-Silhouette-measure-k-%d-feature-space.pdf',k);
  end
  saveas(gcf,fName,'pdf')
...
%saveas(gcf,'FIGS/Feldmeyer-Silhouette-measure-summary.pdf','pdf')
printA4('FIGS/Feldmeyer-Silhouette-measure-summary.eps')
...
for nClust = [2 3 5];
  ...
  fName = sprintf('RESULTS/Feldmeyer-BlindClustering-latex-n-%d.tex', nClust);
  fid = fopen(fName,'w');
  fprintf(fid,str2);
  fclose(fid);
end
